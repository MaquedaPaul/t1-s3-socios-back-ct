...
import {
  ...
  Partner,
  PartnerEmail,
} from '../entities/package.entities';
...
export class PartnerService {
  ...
  constructor(
    ...
    this.engine = new Liquid({
      root: ['public', 'public/components', 'public/layouts', 'public/pages'],
      extname: '.liquid',
    });
  }
  ...
  private createWebsites(partner: CreatePartnerDto, newPartner: Partner) {
    newPartner.websites = partner.websites.map(
      (website) => new PartnerWebsite(website),
    );
  ...
  private createEmails(partner: CreatePartnerDto, newPartner: Partner) {
    newPartner.emails = partner.emails.map((email) => new PartnerEmail(email));
    newPartner.emails.forEach((email) => (email.partner = newPartner));
  }
  private async createParticularMembership(
  ...
  private conversionEnumPartnerType(partnerType: string) {
    if (partnerType === '0') return PartnerType.PLENARY;
    if (partnerType === '1') return PartnerType.ASSOCIATE;
  ...
  async update(id: number, updatePartnerDto: UpdatePartnerDTO) {
    ...
    return this.dataPrint(
      partners,
      `Se actualizo el socio con id: ${id}`,
      'home',
    );
  }
  ...
  async createSeveralPartners(quantity: number) {
    ...
    await this.categoryRepository.save(
      new Category('Supermercado', 'Categoria 1'),
    );
    ...
    for (let i = 1; i <= quantity; i++) {
      this.create(
        new CreatePartnerDto(
          ...
          `Localidad ${i}`,
          `Provincia ${i}`,
          [new PhoneDTO(`CodigoArea ${i}`, `Numero ${i}`, 1)],
          ['asdfasdf@1.com', 'asdfasdf@2.com'],
          ['www.asdfasdf.com', 'www.asdfasdf.com'],
          '0',
          [new CategoryDTO('nombre-categoria', `Categoria ${i}`)],
          12,
          1,
          '2021-01-01',
        ),
